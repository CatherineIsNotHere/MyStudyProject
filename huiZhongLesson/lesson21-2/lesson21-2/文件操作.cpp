...
FILE* openFileForRead(char* fileStr){//打开一个file返回file
	errno_t err = -1;
	FILE* file = nullptr;
...
FILE* openFileForReadBinery(char* fileStr){//打开一个二进制file
	errno_t err = -1;
	FILE* file = nullptr;
...
int tarTheFiles(FILE* srcFiles, FILE* targetFile, int count){//三个参数，源文件名称，目标文件名，源文件个数
